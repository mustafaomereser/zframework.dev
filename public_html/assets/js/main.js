...
$.ajaxSetup({
    ...
    error: e => {
        ...
        if (status == 401) return $('#auth-content .btn-new').click();
...
function init() {
    ...
    $('component:not([installed])').attr('installed', true).each((_, component) => {
        component = $(component);
        component.on('refresh', function () {
            $.get(`/components/${component.attr('data-type')}?${component.attr('data-query') || ''}`, e => (component[0].outerHTML = e, init()));
        }).trigger('refresh');
    });
}
